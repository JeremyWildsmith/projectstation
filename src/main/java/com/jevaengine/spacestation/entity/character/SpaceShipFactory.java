...
public class SpaceShipFactory implements IRpgCharacterFactory
{
    private final Logger m_logger = LoggerFactory.getLogger(SpaceShipFactory.class);
    ...
    @Inject
    public SpaceShipFactory(IItemFactory itemFactory, IScriptBuilderFactory scriptBuilderFactory, IAudioClipFactory audioClipFactory, IAnimationSceneModelFactory animationSceneModelFactory, IConfigurationFactory configurationFactory, IUIStyleFactory styleFactory, IParticleEmitterFactory particleEmitterFactory)
    {
    ...
    private Map<SpaceShipAnimation, IAnimationSceneModelAnimation> createAnimations(IAnimationSceneModel model)
    {
        Map<SpaceShipAnimation, IAnimationSceneModelAnimation> result = new HashMap<>();

        for(SpaceShipAnimation a : SpaceShipAnimation.values())
        {
    ...
    private DefaultActionModel createActionModel(IAnimationSceneModel animationModel, IImmutableAttributeSet attributes, IImmutableLoadout loadout)
    {
        Map<SpaceShipAnimation, IAnimationSceneModelAnimation> animations = createAnimations(animationModel);
        DefaultActionModel actionModel = new DefaultActionModel(animationModel);

        for(SpaceShipAction a : SpaceShipAction.values())
            actionModel.addAction(a.build(animations, attributes, loadout));
    ...
    private IActionSceneModel createModel(URI name, String model, @Nullable String statusStyle, String blood, AttributeSet attributes, DefaultLoadout loadout)
    {
        //Initialize model...
        ...
        try
        {
            ...
            actionModel = createActionModel(spriteModel, attributes, loadout);
    ...
    protected ISpaceCharacterStatusResolverFactory createStatusResolverFactory(URI configContext, UsrCharacterDeclaration characterDecl) throws URISyntaxException, UIStyleConstructionException {
        return new SpaceShipStatusResolverFactory();
    }
    ...
    public IRpgCharacter create(String instanceName, @Nullable URI config, IImmutableVariable auxConfig) throws CharacterCreationException
    {
        ...
        try
        {
            ...
            IActionSceneModel model = createModel(name, characterDecl.model, characterDecl.statusStyle, characterDecl.blood, attributes, loadout);
            ...
            SpaceShip.ICorpseProducer corpseProducer = () -> {
                try {
